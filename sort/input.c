...
void sort_base_array(void (*algorithm) (double* arr, double arr_size), const char* name)
{
    ...
    printf("\n%s - Vector Size: %d - CPU time : %.6f seconds\n", name, LENGTH_BASE_ARRAY, cpu_time);
...
void sort_uniform_arrays(void (*algorithm) (double* arr, double arr_size), const char* name)
{
    ...
    num_input_files = sizeof(uniform_input) / sizeof(int);

    for (input = 0; input < num_input_files; input++)
    {
        for (test = 1; test <= NUM_TESTS_PER_FILE; test++)
        {
            vec_size = uniform_input[input];
            sprintf(file_path, FILE_PATH_PATTERN, vec_size, test);
        ...
        printf("%s [%02d/%d] - Vector Size: %d - Average CPU time : %.6f seconds\n",
